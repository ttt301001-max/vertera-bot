...
TEXTS = {
    "ru": {
        ...
        "anketa_birthday": "🎂 Введите вашу дату рождения (например: 15.03.1990):",
    ...
    "tk": {
        ...
        "anketa_birthday": "🎂 Doglan günüňizi giriziň (mysal: 15.03.1990):",
    ...
    "uz": {
        ...
        "anketa_birthday": "🎂 Tug'ilgan kuningizni kiriting (masalan: 15.03.1990):",
...
async def _job_morning_greet(context):
    """Каждое утро в 9:00 — мотивационное сообщение активным партнёрам."""
    partners = _jload(_PARTNERS_F)
    users    = _jload(_USERS_F)
    ...
    morning_msgs = {
        "ru": (
            ...
            "📊 Твой прогресс:\n"
            "• Обучение: день {learn}/7\n"
            "• Маркетинг: день {mkt}/7\n"
            "• Команда: {refs} чел.\n\n"
            "💪 Сделай сегодня один шаг вперёд — и результат не заставит ждать! 🌿"
        ),
        "tk": (
            "🌿 *Hoş irden, {name}!*\n\n"
            "Täze gün — ösüş üçin täze mümkinçilikler.\n\n"
            "📊 Ösüşiň:\n"
            "• Okuw: {learn}/7 gün\n"
            "• Marketing: {mkt}/7 gün\n"
            "• Topar: {refs} adam\n\n"
            "💪 Bu gün bir ädim öňe git! 🌿"
        ),
        "uz": (
            "🌿 *Xayrli tong, {name}!*\n\n"
            "Yangi kun — o'sish uchun yangi imkoniyatlar.\n\n"
            "📊 Rivojlanishing:\n"
            "• O'qitish: {learn}/7 kun\n"
            "• Marketing: {mkt}/7 kun\n"
            "• Jamoa: {refs} kishi\n\n"
            "💪 Bugun bir qadam oldinga boring! 🌿"
        ),
    }

    for uid_str, pdata in partners.items():
        try:
            uid   = int(uid_str)
            lang  = pdata.get("lang", "ru")
            pname = pdata.get("name", "").split()[0] or "партнёр"
            learn = progress_get(uid)
            mkt   = mkt_progress_get(uid)
            refs  = ref_count(uid) if "ref_count" in dir() else 0
            tmpl  = morning_msgs.get(lang, morning_msgs["ru"])
            await context.bot.send_message(
                chat_id=uid,
                text=tmpl.format(name=pname, learn=learn, mkt=mkt, refs=refs),
                parse_mode="Markdown"
            )
        except Exception:
            pass

async def _job_birthday_check(context):
    """Каждый день проверяем дни рождения в анкетах и поздравляем."""
    today_str = _dt.date.today().strftime("%d.%m")
    users = _jload(_USERS_F)

    bday_msgs = {
        "ru": "🎂 *С Днём Рождения, {name}!*\n\nОт всей команды Dream Team Vertera желаем крепкого здоровья, счастья и больших успехов в бизнесе! 🌿🥳",
        "tk": "🎂 *Doglan günüň bilen, {name}!*\n\nDream Team Vertera toparynyň adyndan saglyk, bagt we işde uly üstünlikler arzuw edýäris! 🌿🥳",
        "uz": "🎂 *Tug'ilgan kuning bilan, {name}!*\n\nDream Team Vertera jamoasidan mustahkam sog'liq, baxt va katta muvaffaqiyatlar tilaymiz! 🌿🥳",
    }
    ...
    for uid_str, udata in users.items():
        try:
            ...
            if not bday:
                continue
            # Parse day.month from birthday (DD.MM.YYYY)
            parts = bday.replace("/", ".").replace("-", ".").split(".")
            if len(parts) >= 2:
                bday_dm = f"{parts[0].zfill(2)}.{parts[1].zfill(2)}"
                if bday_dm == today_str:
                    lang  = udata.get("lang", "ru")
                    name  = udata.get("name", "").split()[0] or "друг"
                    tmpl  = bday_msgs.get(lang, bday_msgs["ru"])
                    await context.bot.send_message(
                        chat_id=int(uid_str),
...
async def _job_backup(context):
    """Раз в день в 03:00 — бэкап всех данных в Sheets."""
    try:
        import json as _json
        today_str = _dt.date.today().strftime("%d.%m.%Y")
        backup_data = {
            "partners": _jload(_PARTNERS_F),
            "progress": _jload(_PROG_F),
            "mkt":      _jload(_MKTPROG_F),
            "quiz":     _jload(_QUIZ_F),
            "users":    _jload(_USERS_F),
        }
        async with httpx.AsyncClient(follow_redirects=True) as c:
            await c.post(GOOGLE_SHEET_URL, json={
                "type":   "backup",
                "date":   today_str,
                "data":   _json.dumps(backup_data, ensure_ascii=False),
            }, timeout=30)
        logger.info(f"✅ Бэкап выполнен: {today_str}")
    except Exception as e:
        logger.error(f"backup job: {e}")

async def _job_stats(context):
    """Раз в неделю в воскресенье — статистика для админа."""
    try:
        partners = _jload(_PARTNERS_F)
        users    = _jload(_USERS_F)
        today    = _dt.date.today()
        # Считаем новых пользователей за 7 дней (у кого нет поля date — пропускаем)
        new_users = len(users)  # общее количество
        learn_days = [progress_get(int(uid)) for uid in partners]
        avg_learn  = round(sum(learn_days) / len(learn_days), 1) if learn_days else 0
        stuck_d4   = sum(1 for d in learn_days if d == 4)

        await context.bot.send_message(
            chat_id=MANAGER_CHAT_ID,
            text=(
                f"📊 *Еженедельная статистика Vertera Bot*\n"
                f"_{today.strftime('%d.%m.%Y')}_\n\n"
                f"👥 Всего пользователей: *{new_users}*\n"
                f"🤝 Партнёров: *{len(partners)}*\n"
                f"📚 Средний день обучения: *{avg_learn}/7*\n"
                f"⚠️ Застряли на дне 4: *{stuck_d4}* чел.\n\n"
                f"_Для подробного отчёта откройте Google Sheets_"
            ),
            parse_mode="Markdown"
        )
    except Exception as e:
        logger.error(f"stats job: {e}")
...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    partners = _jload(_PARTNERS_F)
    users    = _jload(_USERS_F)
    learn_days = [progress_get(int(uid)) for uid in partners if uid.isdigit()]
    mkt_days   = [mkt_progress_get(int(uid)) for uid in partners if uid.isdigit()]
    avg_l = round(sum(learn_days) / len(learn_days), 1) if learn_days else 0
    avg_m = round(sum(mkt_days)   / len(mkt_days),   1) if mkt_days   else 0
    stuck = sum(1 for d in learn_days if d == 4)
    done7 = sum(1 for d in learn_days if d >= 7)

    import datetime as _dtt
    today = _dtt.date.today().strftime("%d.%m.%Y")

    await update.message.reply_text(
        f"📊 *Статистика Vertera Bot*\n_{today}_\n\n"
        f"👥 Всего пользователей: *{len(users)}*\n"
        f"🤝 Партнёров: *{len(partners)}*\n\n"
        f"📚 *Обучение:*\n"
        f"  • Средний день: *{avg_l}/7*\n"
        f"  • Завершили: *{done7}* чел.\n"
        f"  • Застряли на дне 4: *{stuck}* чел.\n\n"
        f"📊 *Маркетинг:*\n"
        f"  • Средний день: *{avg_m}/7*\n\n"
        f"_Подробности — в Google Sheets_ 📋",
        parse_mode="Markdown"
    )

def setup_daily_jobs(app):
    """Регистрирует все ежедневные задачи в job_queue."""
    if not app.job_queue:
        logger.warning("job_queue недоступен — daily jobs не запущены")
        return
    # Утреннее сообщение: каждый день в 09:00 UTC+5 (04:00 UTC)
    app.job_queue.run_daily(
        _job_morning_greet,
        time=_dt.time(4, 0, 0),  # 09:00 Ашхабад/Ташкент
        name="morning_greet",
    )
    # Поздравления с ДР: каждый день в 09:05 UTC
    app.job_queue.run_daily(
        _job_birthday_check,
        time=_dt.time(4, 5, 0),
        name="birthday_check",
    )
    # Бэкап: каждый день в 03:00 UTC (08:00 по Ашхабаду)
    app.job_queue.run_daily(
        _job_backup,
        time=_dt.time(22, 0, 0),  # 03:00 следующего дня по Ашхабаду
        name="daily_backup",
    )
    # Статистика: каждое воскресенье в 09:00
    app.job_queue.run_daily(
        _job_stats,
        time=_dt.time(4, 0, 0),
    ...
    logger.info("✅ Daily jobs запущены")
...
async def anketa_interest(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сохраняем birthday в users кэше для поздравлений
    birthday_val = context.user_data.get("birthday", "")
    if birthday_val:
        users_d = _jload(_USERS_F)
        uid_str = str(update.effective_user.id)
        if uid_str in users_d:
            users_d[uid_str]["birthday"] = birthday_val
